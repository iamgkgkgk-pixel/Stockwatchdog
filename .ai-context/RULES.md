...
### R-05: 当前与历史分位必须使用同口径、等频的本地参照

- 经 `DataQuality.comparableHistory` 过滤后，使用 `referenceValues` 按月取最后样本；不可将旧月点与近期日点直接等权混合。
- API分位仅展示参考，不替代本地分位；代理指数不能与目标指数旧历史或锚点混算。
- 图表是事后统一基准重算，不得当作当时可执行信号或收益回测证据。
- R-02的原有买卖阈值不变；未核验、过期或辅助项缺失时显示带日期/覆盖率的参考评分，不直接给仓位指令。只有主数据缺失、身份不符或无可比基准才不能计算。
- 独立估值历史位置只使用PE或债息，不复用完整买卖信号的必需字段门禁。参考评分不伪装成全维度评分或安全概率。
- 参见 DEC-014（取代DEC-013的一刀切暂停）；原始JSON不被过滤操作修改或删除。
